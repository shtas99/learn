...
n = int(input())
for x in range (1, n+1):          # внешний цикл который переберет все числа от 1 до n
    count = 0
    for i in range (1, x+1):      # внутренний цикл который переберет все возможные делители и сохранит их количество в count
        if x % i == 0:
            count += 1
    print(f"{x} {'+' * count}")   # выведем ответ в консоль